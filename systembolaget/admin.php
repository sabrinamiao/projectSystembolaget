...
function login() {
  ...
  $results = array();
  $results['pageTitle'] = "Admin Login | SystemBolaget";
...
function newSortiment() {

  $results = array();
  $results['pageTitle'] = "New Sortiment";
  $results['formAction'] = "newSortiment";

  if ( isset( $_POST['saveChanges'] ) ) {

    // User has posted the Sortiment edit form: save the new Sortiment
    $Sortiment = new Sortiment;
    $Sortiment->storeFormValues( $_POST );
    $Sortiment->insert();
    if ( isset( $_FILES['image'] ) ) $Sortiment->storeUploadedImage( $_FILES['image'] );
    header( "Location: admin.php?status=changesSaved" );
    ...
    // User has not posted the Sortiment edit form yet: display the form
    $results['Sortiment'] = new Sortiment;
    require( TEMPLATE_PATH . "/admin/editSortiment.php" );
  }

}


function editSortiment() {

  $results = array();
  $results['pageTitle'] = "Edit Sortiment";
  $results['formAction'] = "editSortiment";

  if ( isset( $_POST['saveChanges'] ) ) {

    // User has posted the Sortiment edit form: save the Sortiment changes

    if ( !$Sortiment = Sortiment::getById( (int)$_POST['SortimentId'] ) ) {
      header( "Location: admin.php?error=SortimentNotFound" );
      return;
    }

    $Sortiment->storeFormValues( $_POST );
    if ( isset($_POST['deleteImage']) && $_POST['deleteImage'] == "yes" ) $Sortiment->deleteImages();
    $Sortiment->update();
    if ( isset( $_FILES['image'] ) ) $Sortiment->storeUploadedImage( $_FILES['image'] );
    header( "Location: admin.php?status=changesSaved" );
    ...
    // User has cancelled their edits: return to the Sortiment list
    header( "Location: admin.php" );
  } else {
...
function listSortiments() {
  $results = array();
  $data = Sortiment::getList();
  $results['Sortiments'] = $data['results'];
  $results['totalRows'] = $data['totalRows'];
  $results['pageTitle'] = "All Sortiments";

  if ( isset( $_GET['error'] ) ) {
    if ( $_GET['error'] == "SortimentNotFound" ) $results['errorMessage'] = "Error: Sortiment not found.";
  }

  if ( isset( $_GET['status'] ) ) {
    if ( $_GET['status'] == "changesSaved" ) $results['statusMessage'] = "Your changes have been saved.";
    if ( $_GET['status'] == "SortimentDeleted" ) $results['statusMessage'] = "Sortiment deleted.";
  }

  require( TEMPLATE_PATH . "/admin/listSortiments.php" );
}
